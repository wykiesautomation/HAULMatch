...
class FinanceDocument(Base):
 __tablename__='finance_documents';id=Column(Integer,primary_key=True);reference=Column(String(64),unique=True,index=True);account_id=Column(Integer);payment_id=Column(Integer);document_type=Column(String(40));description=Column(String(250));amount_cents=Column(Integer,default=0);storage_key=Column(String(300));sha256=Column(String(64));created_at=Column(DateTime,default=datetime.utcnow)
class WalletAdjustmentApproval(Base):
 __tablename__='wallet_adjustment_approvals';id=Column(Integer,primary_key=True);reference=Column(String(64),unique=True);account_id=Column(Integer);credits=Column(Integer);reason=Column(String(250));status=Column(String(40),default='PENDING');requested_by=Column(Integer);approved_by=Column(Integer);created_at=Column(DateTime,default=datetime.utcnow)
