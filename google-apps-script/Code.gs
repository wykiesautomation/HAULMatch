...
/* HAULMATCH COMPLETE ADMIN V2 + GOOGLE AUTH START */
function adminConsoleRoutePost_(p) {
  var supplied = String(p.adminKey || '');
  if (ADMIN_KEY && supplied === ADMIN_KEY) return adminConsoleV2Post_(p);
  return adminGoogleActionPost_(p);
}
function adminConsoleV2_(){return adminConsoleV2Render_({key:'',message:'',error:'',data:null,googleAdmin:null});}
function adminConsoleV2Post_(p){
  var key=String(p.adminKey||''),action=String(p.consoleAction||'LOAD'),ref=String(p.reference||''),credits=Number(p.credits||0),reason=String(p.reason||''),cost=Number(p.cost||0);
  try{
    checkAdmin_(key);var ss=SpreadsheetApp.openById(SHEET_ID),message='Admin data loaded.';
    if(['REQUEST_PUBLISH','REQUEST_CLOSE','REQUEST_REOPEN','REQUEST_UNPUBLISH'].indexOf(action)>=0){
      var map={REQUEST_PUBLISH:'PUBLISHED',REQUEST_CLOSE:'CLOSED',REQUEST_REOPEN:'NEW',REQUEST_UNPUBLISH:'NEW'};
      adminAction_(ss,{key:key,reference:ref,status:map[action]});message=ref+' updated to '+map[action]+'.';
    }else if(action==='REQUEST_COST'){
      gSetCost_(ss,{key:key,reference:ref,cost:cost});message=ref+' lead cost set to '+cost+'.';
    }else if(['TRANSPORTER_APPROVE','TRANSPORTER_MORE_INFO','TRANSPORTER_REJECT','TRANSPORTER_BLOCK'].indexOf(action)>=0){
      var sm={TRANSPORTER_APPROVE:'APPROVED',TRANSPORTER_MORE_INFO:'MORE INFO REQUIRED',TRANSPORTER_REJECT:'REJECTED',TRANSPORTER_BLOCK:'BLOCKED'};
      if(action==='TRANSPORTER_BLOCK') moderationAction_(ss,{key:key,reference:ref,status:'BLOCKED'});
      else transporterDecision_(ss,{key:key,reference:ref,status:sm[action]});
      message=ref+' updated to '+sm[action]+'.';
    }else if(action==='WALLET_ADJUST'){
      if(!reason||!credits)throw Error('Wallet adjustment needs non-zero credits and a reason.');
      g6WalletAdjust_(ss,{key:key,transporterReference:ref,credits:credits,reason:reason});message=ref+' wallet adjusted by '+credits+' credits.';
    }else if(action!=='LOAD')throw Error('Unsupported Admin action');
    return adminConsoleV2Render_({key:key,message:message,error:'',data:adminConsoleV2Data_(key),googleAdmin:null});
  }catch(e){return adminConsoleV2Render_({key:key,message:'',error:e.message||String(e),data:null,googleAdmin:null});}
}
function adminConsoleV2Data_(key){
  checkAdmin_(key);var ss=SpreadsheetApp.openById(SHEET_ID),r=sheet_(ss,'Transport Requests',RH),t=sheet_(ss,'Transporter Applications',TH),w=gWallets_(ss),u=gUnlocks_(ss),q=sheet_(ss,'Quotes',QH),a=gAppointmentsSheet_(ss),f=gFeedbackSheet_(ss);
  var rv=r.getDataRange().getDisplayValues(),tv=t.getDataRange().getDisplayValues(),wv=w.getDataRange().getDisplayValues();
  ...
  var transporters=tv.slice(1).reverse().slice(0,100).map(function(x){var wr=find_(w,x[1],1),bal=wr?Number(w.getRange(wr,3).getValue()||0):0;return{reference:x[1],contact:x[2],company:x[3],email:x[4],services:x[7],status:x[16],risk:x[17]||0,moderation:x[18],credits:bal};});
  return{published:requests.filter(function(x){return x.status==='PUBLISHED'}).length,pendingRequests:requests.filter(function(x){return x.status==='NEW'}).length,pendingTransporters:transporters.filter(function(x){return x.status==='PENDING REVIEW'}).length,credits:wv.slice(1).reduce(function(n,x){return n+Number(x[2]||0)},0),unlocks:Math.max(0,u.getLastRow()-1),quotes:Math.max(0,q.getLastRow()-1),appointments:Math.max(0,a.getLastRow()-1),issues:f.getLastRow()>1?f.getRange(2,8,f.getLastRow()-1,1).getDisplayValues().flat().filter(function(x){return x==='OPEN'}).length:0,requests:requests,transporters:transporters};
}
function adminConsoleV2Render_(m){
  var e=adminHtmlEscape_,url=ScriptApp.getService().getUrl(),key=String(m.key||''),d=m.data;
  function post(action,ref,label,cls,extra){return '<form method="post" action="'+e(url)+'"><input type="hidden" name="formType" value="adminConsoleForm"><input type="hidden" name="adminKey" value="'+e(key)+'"><input type="hidden" name="consoleAction" value="'+e(action)+'"><input type="hidden" name="reference" value="'+e(ref)+'">'+(extra||'')+'<button class="'+cls+'" type="submit">'+e(label)+'</button></form>';}
  var content='';
  if(d){
    var req=d.requests.map(function(x){var buttons='';if(x.status==='PUBLISHED')buttons=post('REQUEST_UNPUBLISH',x.reference,'Unpublish','ghost')+post('REQUEST_CLOSE',x.reference,'Close','danger');else if(x.status==='CLOSED')buttons=post('REQUEST_REOPEN',x.reference,'Reopen','blue');else if(x.status==='NEW')buttons=post('REQUEST_PUBLISH',x.reference,'Publish','primary')+post('REQUEST_CLOSE',x.reference,'Close','danger');else buttons=post('REQUEST_REOPEN',x.reference,'Reopen','blue');var cost='<input class="smallinput" name="cost" type="number" min="1" max="5" value="'+e(x.cost)+'">';return '<article class="row"><div><b>'+e(x.reference)+' | '+e(x.type)+'</b><p>'+e(x.route)+'</p><span>'+e(x.status)+' | Risk '+e(x.risk)+' | '+e(x.cost)+' credits | '+e(x.responses)+'/6</span></div><div class="acts">'+buttons+post('REQUEST_COST',x.reference,'Set Cost','blue',cost)+'</div></article>';}).join('')||'<div class="empty">No requests.</div>';
    var trs=d.transporters.map(function(x){var buttons=x.status==='APPROVED'?post('TRANSPORTER_BLOCK',x.reference,'Block','danger'):post('TRANSPORTER_APPROVE',x.reference,'Approve + Wallet','primary')+post('TRANSPORTER_MORE_INFO',x.reference,'More Info','ghost')+post('TRANSPORTER_REJECT',x.reference,'Reject','danger');var adj='<input class="smallinput" name="credits" type="number" placeholder="+/-"><input class="reason" name="reason" placeholder="Reason" required>';return '<article class="row"><div><b>'+e(x.reference)+' | '+e(x.company||x.contact)+'</b><p>'+e(x.email)+' | '+e(x.services)+'</p><span>'+e(x.status)+' | '+e(x.credits)+' credits | Risk '+e(x.risk)+'</span></div><div class="acts">'+buttons+post('WALLET_ADJUST',x.reference,'Adjust Credits','blue',adj)+'</div></article>';}).join('')||'<div class="empty">No transporter applications.</div>';
    content='<div class="kpis"><div><b>'+d.pendingRequests+'</b>Pending requests</div><div><b>'+d.published+'</b>Published</div><div><b>'+d.pendingTransporters+'</b>Pending transporters</div><div><b>'+d.credits+'</b>Total credits</div><div><b>'+d.unlocks+'</b>Unlocks</div><div><b>'+d.issues+'</b>Issues</div></div><nav><button onclick="showTab(\'requests\')">Requests</button><button onclick="showTab(\'transporters\')">Transporters & Credits</button><button onclick="showTab(\'modules\')">Module Health</button></nav><section id="requests" class="tab active"><h2>Lead Control</h2>'+req+'</section><section id="transporters" class="tab"><h2>Transporter Approval & Wallets</h2>'+trs+'</section><section id="modules" class="tab"><h2>Module Health</h2><div class="modules"><div>Google Admin / Apps Script <b>ONLINE</b></div><div>Public Leads <b>CONNECTED</b></div><div>Wallet Ledger <b>CONNECTED</b></div><div>Unlocks <b>'+d.unlocks+'</b></div><div>Quotes <b>'+d.quotes+'</b></div><div>Appointments <b>'+d.appointments+'</b></div><div>Uploads <b>CONNECTED</b></div><div>PayFast <b>SANDBOX</b></div></div></section>';
  }
  var html='<!doctype html><html><head><base target="_top"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HaulMatch Complete Admin</title><style>:root{--n:#071827;--p:#10334f;--l:#b7f52f;--c:#22d3ee;--w:#edf7ff;--m:#a8bfd0}*{box-sizing:border-box}body{margin:0;background:var(--n);color:var(--w);font:15px Arial}.top{background:#ffc857;color:#172000;text-align:center;padding:11px;font-weight:900}.wrap{max-width:1450px;margin:auto;padding:32px 4%}h1{font-size:52px;margin:8px 0 24px}.login,.row,.kpis>div,.modules>div,.empty{background:var(--p);border:1px solid #ffffff18;border-radius:15px;padding:16px}.login{display:grid;grid-template-columns:1fr auto;gap:10px}input{background:#071d2e;color:#fff;border:1px solid #49677c;border-radius:9px;padding:11px}button{border:0;border-radius:9px;padding:11px 14px;font-weight:900;cursor:pointer}.primary{background:var(--l);color:#172000}.blue{background:#176fa9;color:#fff}.ghost{background:#25465f;color:#fff}.danger{background:#7b2838;color:#fff}.msg{color:#a8ffbe;margin:14px 0}.err{color:#ffadb6}.kpis{display:grid;grid-template-columns:repeat(6,1fr);gap:10px;margin:17px 0}.kpis b{display:block;font-size:28px}nav{display:flex;gap:8px;margin:18px 0}.tab{display:none}.tab.active{display:block}.row{display:grid;grid-template-columns:1fr auto;gap:12px;margin:10px 0}.row p,.row span{color:var(--m)}.acts{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.acts form{display:flex;gap:5px}.smallinput{width:66px}.reason{width:120px}.modules{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.modules b{display:block;color:var(--l);margin-top:7px}@media(max-width:900px){.kpis,.modules{grid-template-columns:repeat(2,1fr)}.row,.login{grid-template-columns:1fr}.acts{justify-content:flex-start}}</style><script>function showTab(id){document.querySelectorAll(".tab").forEach(function(x){x.classList.remove("active")});document.getElementById(id).classList.add("active")}</script></head><body><div class="top">SECURE HAULMATCH COMPLETE ADMIN COCKPIT</div><main class="wrap"><h1>Admin Cockpit</h1><form class="login" method="post" action="'+e(url)+'"><input type="hidden" name="formType" value="adminConsoleForm"><input type="hidden" name="consoleAction" value="LOAD"><input name="adminKey" type="password" value="'+e(key)+'" placeholder="Admin key" required><button class="primary">Load Complete Admin</button></form>'+(m.message?'<div class="msg">'+e(m.message)+'</div>':'')+(m.error?'<div class="err">'+e(m.error)+'</div>':'')+content+'</main></body></html>';
  return HtmlService.createHtmlOutput(html).setTitle('HaulMatch Complete Admin');
}
function adminGoogleAllowedEmails_(){var p=PropertiesService.getScriptProperties();var raw=p.getProperty('HAULMATCH_ADMIN_EMAILS')||p.getProperty('HAULMATCH_ADMIN_EMAIL')||'';return raw.split(/[;,\s]+/).map(function(x){return String(x).trim().toLowerCase();}).filter(String);}
...
function adminGoogleReplaceSecret_(output,sessionToken,admin){var html=output.getContent(),safeToken=adminHtmlEscape_(sessionToken),safeEmail=adminHtmlEscape_(admin.email);html=html.replace(/<input name="adminKey" type="password"[^>]*>/,'<input type="hidden" name="adminKey" value="'+safeToken+'">');html=html.replace('Load Complete Admin','Refresh Admin');html=html.replace('<h1>Admin Cockpit</h1>','<h1>Admin Cockpit</h1><p style="color:#22d3ee;font-weight:700">Signed in as '+safeEmail+'</p>');html=html.replace('SECURE HAULMATCH COMPLETE ADMIN COCKPIT','SECURE HAULMATCH GOOGLE ADMIN COCKPIT');return HtmlService.createHtmlOutput(html).setTitle('HaulMatch Google Admin');}
function adminGoogleLoginPost_(p){try{var admin=adminGoogleVerifyIdToken_(String(p.credential||'')),sessionToken=adminGoogleCreateSession_(admin);audit_(SpreadsheetApp.openById(SHEET_ID),'ADMIN_GOOGLE_LOGIN',admin.email,admin.subject);return adminGoogleReplaceSecret_(adminConsoleV2Render_({key:ADMIN_KEY,message:'Google administrator verified.',error:'',data:adminConsoleV2Data_(ADMIN_KEY),googleAdmin:admin}),sessionToken,admin);}catch(e){return adminGoogleError_(e.message||String(e));}}
function adminGoogleActionPost_(p){try{var admin=adminGoogleSession_(p.adminKey),copy={};Object.keys(p).forEach(function(k){copy[k]=p[k];});copy.adminKey=ADMIN_KEY;audit_(SpreadsheetApp.openById(SHEET_ID),'ADMIN_GOOGLE_ACTION',String(p.reference||''),admin.email+' | '+String(p.consoleAction||'LOAD'));return adminGoogleReplaceSecret_(adminConsoleV2Post_(copy),p.adminKey,admin);}catch(e){return adminGoogleError_(e.message||String(e));}}
function adminGoogleError_(message){var returnUrl='https://haulmatch.wykiesautomation.co.za/admin/';return HtmlService.createHtmlOutput('<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HaulMatch Admin Access</title><style>body{margin:0;background:#071827;color:#edf6ff;font:16px Arial;display:grid;place-items:center;min-height:100vh}.card{max-width:620px;background:#102f49;border-radius:18px;padding:28px;text-align:center}.error{color:#ffadb4}a{display:inline-block;margin-top:15px;padding:12px 16px;border-radius:10px;background:#b7f52f;color:#172000;text-decoration:none;font-weight:900}</style></head><body><div class="card"><h1>Admin access denied</h1><p class="error">'+adminHtmlEscape_(message)+'</p><a href="'+returnUrl+'">Return to Google Admin Sign-In</a></div></body></html>').setTitle('HaulMatch Admin Access');}
/* HAULMATCH COMPLETE ADMIN V2 + GOOGLE AUTH END */
